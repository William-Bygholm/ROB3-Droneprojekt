...
while True:
    ...
    for det_box, score in zip(final_boxes, final_scores):
        ...
        labels_all.append(1 if matched else 0)
        scores_all.append(score)

    # ------ Add FN ------
    for used in gt_used:
        if not used:
            labels_all.append(1)      # GT exists
            scores_all.append(-999.0) # guaranteed FN
...
tn = 0  # detection task = no true negatives ever
...
plt.scatter([fp / (fp + tn + 1e-9)], [tp / (tp + fn + 1e-9)], color='red', s=80, label=f"Thr={best_threshold:.4f}")
plt.title("ROC Curve")
...
plt.scatter([recall], [precision], color='red', s=80, label=f"Thr={best_threshold:.4f}")
